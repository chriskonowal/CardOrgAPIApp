...
export default {
  ...
  methods: {
    readDataFromAPI() {
      ...
      console.log(request);
      axios
        .post("http://localhost:54421/api/admin/locations", request)
        .then((response) => {
          console.log(response.data);
          if (response.data.isSuccessful) {
            this.items = response.data.value.locations;
    ...
    openNewItemDialog() {
      this.editTitle = "Add Location Company";
      this.editedItem = {};
      this.editDialog = true;
    },
    editItem(item) {
      console.log(item);
      this.editTitle = "Edit Location Company";
    ...
    editSave() {
      ...
      var request = {
        id: this.editedItem.locationId,
        name: this.editedItem.name,
      };
      console.log(request);
      axios({
        method: "post", //you can set what request you want to be
        url: "http://localhost:54421/api/admin/locations/save",
        data: request,
      }).then((response) => {
        console.log(response.data);
        if (!response.data.isSuccessful) {
          ...
          if (isEdit) {
            ...
            this.infoDialogTitleMessage = "Edit Location Company";
          } else {
            this.infoDialogMessage = "Add successful!";
            this.infoDialogTitleMessage = "Add Location Company";
          }
        }
      });
    },
    ...
    deleteItemConfirm() {
      ...
      axios({
        method: "post", //you can set what request you want to be
        url: "http://localhost:54421/api/admin/locations/delete",
        data: request,
      }).then((response) => {
        console.log(response.data);
        if (!response.data.isSuccessful) {
