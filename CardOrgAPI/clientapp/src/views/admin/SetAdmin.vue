...
export default {
  ...
  methods: {
      ...
      axios
        ...
        .then((response) => {
          ...
          if (response.data.isSuccessful) {
            this.items = response.data.value.sets;
    ...
    openNewItemDialog() {
      this.editTitle = "Add Set";
      this.editedItem = {};
      this.editDialog = true;
    },
    editItem(item) {
      console.log(item);
      this.editTitle = "Edit Set";
      this.editedItem = item;
      this.editDialog = true;
    },
    editSave() {
      this.clearAddMessage();
      var isEdit = this.editedItem.setId > 0;
      var validEdit = this.$refs.editForm.validate();
      if (!validEdit) {
        return;
      }
      var request = {
        id: this.editedItem.setId,
        name: this.editedItem.name,
      };
      console.log(request);
      axios({
        method: "post", //you can set what request you want to be
        url: process.env.VUE_APP_ROOT_API + "admin/sets/save",
        data: request,
      }).then((response) => {
        console.log(response.data);
        if (!response.data.isSuccessful) {
          ...
          if (isEdit) {
            ...
            this.infoDialogTitleMessage = "Edit Set";
          } else {
            this.infoDialogMessage = "Add successful!";
            this.infoDialogTitleMessage = "Add Set";
          }
        }
      });
    },
    ...
    deleteItemConfirm() {
      ...
      axios({
        method: "post", //you can set what request you want to be
        url: process.env.VUE_APP_ROOT_API + "admin/sets/delete",
        data: request,
      }).then((response) => {
        console.log(response.data);
        if (!response.data.isSuccessful) {
          ...
          this.infoDialogTitleMessage = "Delete Set";
          this.closeDelete();
        }
      });
    },
